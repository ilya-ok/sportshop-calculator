...
37. **`ssc-res-row-leftover` — «Остаток материала»** — строка показывает суммарную площадь отходов в м². Лейбл «Остаток материала:», спан `.ssc-res-leftover`. Расположена между «Рулонов в раскладке» и «Длина швов». Показывается если `leftoverArea > 0`. Текст: `"X.XX м² (пров.: Y.YY м²)"`.
   - **`leftoverArea`** = `rollLeftover × rW + (wRem > 0.001 ? (rW - wRem) × L : 0)` — торец последнего рулона + срезанная вдоль полоса. `wRem = W % rW`.
   - **`verifyLeftover`** = `Math.ceil(stripsCount × L / rL) × rW × rL − area` — площадь купленных рулонов (по фактической раскладке) минус площадь комнаты. Должна совпадать с `leftoverArea`. Использует `ceil(stripsCount × L / rL)`, а НЕ `ceil(area / (rW × rL))` — последнее даёт теоретический минимум рулонов по площади и не совпадает с фактическим количеством.
   - В `_calculateSimple` оба значения вычисляются только внутри `if (simpleRollsEnabled)`, в результат передаются как `|| 0`.
